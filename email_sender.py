...
def send_highlight_email(
    *,
    ...
    # API 요청 구성 (test_api.py 형식)
    url = f"{EMAIL_API_BASE_URL}/api/email/users/{urllib.parse.quote(sender, safe='@')}/emails"
    ...
    # 수신인 여러 명: toRecipients[] 파라미터를 반복
    ...
    log_fn(f"이메일 발송 중: {sender} -> {', '.join(recipients)}", "INFO")
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=30) as res:
            ...
            try:
                ...
                log_fn(f"이메일 발송 성공: {json.dumps(result, ensure_ascii=False)[:200]}", "INFO")
            except json.JSONDecodeError:
                log_fn(f"이메일 발송 성공: {response_data[:200]}", "INFO")
        return True
        ...
        try:
            ...
            pass
        log_fn(f"이메일 발송 실패 (HTTP {e.code}): {error_body}", "ERROR")
        return False
    except Exception as e:
        log_fn(f"이메일 발송 실패: {e}", "ERROR")
        return False
